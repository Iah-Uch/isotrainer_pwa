...
document.getElementById('loadCsvBtn').addEventListener('click', ()=>{
  ...
  if (!(state.device && state.device.gatt?.connected)){ err.textContent = 'Connect a device first.'; err.classList.remove('hidden'); return; }
  try{ const session = parseTrainingCsv(document.getElementById('csvInput').value); startTraining(session); }
