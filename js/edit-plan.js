...
function render() {
  ...
  if (selAll)
    selAll.checked =
      selected.size === (working?.stages?.length || 0) && selected.size > 0;
  updateAdjTimeButtons();
}

function recalcTotal() {
  if (!working) return;
  working.totalDurationSec = working.stages.reduce(
    (a, s) => a + clamp(Math.round(s.durationSec), 0, 86400),
    0,
  );
}
...
function validate() {
  ...
  for (const s of working.stages) {
    if (!(s.durationSec >= 0)) {
      setError("Duração inválida em um dos estágios.");
      return false;
    }
    if (!(s.upper > s.lower)) {
      setError("Limites inválidos: superior deve ser maior que inferior.");
      return false;
    }
  }
  setError("");
...
function onClick(e) {
  ...
  if (act === "addLower") {
    ...
    working.stages[i].lower = clamp(
      Math.round(working.stages[i].lower + d),
      minV,
      maxV,
    );
    render();
  ...
  if (act === "addUpper") {
    ...
    working.stages[i].upper = clamp(
      Math.round(working.stages[i].upper + d),
      minV,
      maxV,
    );
    render();
...
function updateAdjTimeButtons() {
  const btns = Array.from(
    document.querySelectorAll('button[data-act="adjTime"]'),
  );
  ...
  for (const b of btns) {
    ...
    for (const idx of sel) {
      ...
      const cur = working.stages[idx].durationSec;
      const next = cur + d;
      if (!(next >= caps.timeMin && next <= caps.timeMax)) {
        can = false;
        break;
      }
    }
    ...
    if (b.disabled)
      b.title = "Alguns estágios selecionados atingiram o limite de 25%";
...
export function startWithEditedPlan() {
  ...
  if (Number.isFinite(sessionCopy.idx) && !Number.isFinite(sessionCopy.planIdx))
...
document.addEventListener("click", (e) => {
  if (document.getElementById("editPlanScreen")?.classList.contains("hidden"))
    return;
  onClick(e);
});
...
document
  ...
  ?.addEventListener("click", startWithEditedPlan);
