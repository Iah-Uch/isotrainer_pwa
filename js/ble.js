  ...
  try {
    if (typeof navigator.bluetooth.getAvailability === "function") {
      ...
      if (!avail) {
        ...
        document.getElementById("status").textContent =
          "Adaptador Bluetooth possivelmente indisponível. Você ainda pode tentar conectar.";
      }
    }
  } catch {
...
export async function connectToDevice() {
  ...
  try {
    ...
    try {
      ...
      document.getElementById("status").textContent =
        "Tentando modo compatível (lista completa)...";
      state.device = await navigator.bluetooth.requestDevice({
    ...
    document.getElementById("status").textContent =
      "Conectando ao servidor GATT...";
    state.server = await state.device.gatt.connect();
    document.getElementById("status").textContent =
      `Conectado a ${state.device.name}`;
...
function handleHeartRateMeasurement(value) {
  ...
  if (marker) {
    const period = Math.max(0.3, Math.min(3.0, 60 / Math.max(hr, 1)));
  ...
  try {
    if (
      ...
      let scaleMin =
        state.trainingSession?.sessionBounds?.min ?? first.lower - 20;
      let scaleMax =
        state.trainingSession?.sessionBounds?.max ?? first.upper + 20;
      ...
      if (guideText && iconEl && guideWrap && thumbEl) {
        ...
        if (colorClass === "text-emerald-400")
          thumbEl.classList.add("text-emerald-400");
